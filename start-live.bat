...
echo [3/5] Checking irodori-TTS Server...
...
echo [4/5] Checking OneComme...
call :CheckTcp 127.0.0.1 11180 2
if errorlevel 1 (
  call :StartOneComme
  call :WaitTcp 127.0.0.1 11180 8 "OneComme"
  if errorlevel 1 echo OneComme is still starting. The bot will keep reconnecting.
) else (
  echo OneComme port is already open.
)

echo [5/5] Starting live bot...
...
:StartOneComme
echo OneComme was not found at 127.0.0.1:11180
powershell -NoProfile -ExecutionPolicy Bypass -Command "$exe = Join-Path $env:LOCALAPPDATA 'Programs\OneComme\OneComme.exe'; if (Test-Path -LiteralPath $exe) { Start-Process -FilePath $exe; exit 0 }; $desktop = [Environment]::GetFolderPath('Desktop'); $link = Get-ChildItem -LiteralPath $desktop -Filter '*OneComme*.lnk' -ErrorAction SilentlyContinue | Select-Object -First 1; if ($link) { Start-Process -FilePath $link.FullName; exit 0 }; exit 1"
if errorlevel 1 (
  echo OneComme executable was not found.
)
echo Start OneComme manually if comments do not arrive.
exit /b %errorlevel%
